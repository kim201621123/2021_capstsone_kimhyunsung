...
class ShowVideo(QtCore.QObject):
    ...
    def startVideo(self):
        ...
        while run_video:
            ...
            if self.yolo_flag == 1:     #화면에 얼굴이 있으면    문제 해결!
                ...
                for i in range(len(boxes)):
                    if i in indexes:
                        ...
                        draw.text((x1 + (x2-x1)/2. -30, y2 +20),  self.name_recall(self.label_yo[:-2]), font=font, fill=(b,g,r,a),
            ...
            if self.yolo_flag == 1:     #얼굴은 있는데 욜로 디텍팅이 안되었다면? 모르는 사람이면?
                ...
                h, w = frame.shape[:2]
                draw = ImageDraw.Draw(img_pil)
                draw.text((0, h-160), text_draw, font=font, fill=(b,g,r,a),
                 stroke_width=2,stroke_fill="black")
                """
                ...
                img_pil = Image.fromarray(frame)
                draw = ImageDraw.Draw(img_pil)
                draw.text((x1 - 30, y2 +20),  "미등록인원", font=font, fill=(b,g,r,a),
                    stroke_width=2,stroke_fill="black")
                frame = np.array(img_pil)
                p = 6
                ...
                if curTime - self.formal_time_mobilenet > p :   
                    #self.beepsound(0)
                    self.tts_google("등록되지 않은 인원입니다." ," ")       #gtts로 mp3를 만들고
    ...
    def bg_mp3(self,mp3_name):
        # do some stuff
        download_thread = threading.Thread(target=self.mp3_play, name="Downloader", args=mp3_name)
    ...
    def name_recall(self, yolo_class):
        text_name = ""
        if yolo_class == "K.H.S" :
            text_name = "김현성 님"
        return text_name
    ...
    def is_that_you(self, yolo_class, cur_time, yolo_score,frame): #작동함
        ...
        if self.formal_yolo_class == yolo_class: #사람이 이전 사람과 같다면?
            ...
            if cur_time - 6 > self.formal_time:     #일정시간 이상 이전 사람과 같다 판정되면
                    ...
                    for i in range(len(self.human_table)):      #TypeError: 'int' object is not iterable 
                        ...
                        if (self.human_table[-(i+1)][-3] == yolo_class[:-2]) and self.human_table[-(i+1)][-2] == "True" :   #사람이 리스트에 있고 / 뒤에서부터 사람만 센다
                            ...
                            if abs(time_tmp - time_tmp2) < n :    #리스트에 시간이 n분이상 차이 안나면
                                ...
                                h, w = frame.shape[:2]
                                draw = ImageDraw.Draw(img_pil)
                                #draw.text((0, 0), self.name_recall(yolo_class[:-2]) + "  " +str(n)+"분 이내 측정된 인원입니다.",
                                ...
                                if self.formal_time_yolo == 0: #맨처음 식별되지 않았다면
                                    ...
                                    #self.beepsound(1)
                                    #self.tts_google( str(n)+"분 이내 측정된 인원입니다." , self.name_recall(yolo_class[:-2]))       #gtts로 mp3를 만들고
                                    #self.mp3_play(self.text_voice_g)                        # pygame으로 재생
                ...
                #print("리스트에 넣었습니다.") #
                print("----------------------------------------")
                self.tts_google("등록된 인원입니다." , self.name_recall(yolo_class[:-2]))       #gtts로 mp3를 만들고
                self.mp3_play(self.text_voice_g)                        # pygame으로 재생
                ...
                if yolo_class[-1:] == "n":   # 인물이 헬멧을 안쓰고있으면
                    ...
                    text_draw = text_draw + "\n 헬멧을 착용하세요."
                    #self.beepsound(2)
                    self.tts_google( "헬멧을 착용하세요." , "")       #gtts로 mp3를 만들고
                    self.mp3_play(self.text_voice_g)                        # pygame으로 재생
                #self.tts_google(human)
                if yolo_class[-1:] == "h":   # 인물이 헬멧을 쓰고있으면
                    text_draw = self.name_recall(yolo_class[:-2])+ "  " +" 리스트에 넣었습니다."
                    #print("헬멧 쓰십셔")
                    b,g,r,a = 255,200,50,10
                    fontpath = "fonts/gulim.ttc"
                    text_draw = text_draw + "\n 헬멧 착용했습니다."
                    #self.beepsound(2)
                    self.tts_google( "헬멧 착용했습니다.." , "")       #gtts로 mp3를 만들고
                    self.mp3_play(self.text_voice_g)    
                font = ImageFont.truetype(fontpath, 30)
                img_pil = Image.fromarray(frame)
                h, w = frame.shape[:2]
                draw = ImageDraw.Draw(img_pil)
                draw.text((0, h-160), text_draw, font=font, fill=(b,g,r,a), stroke_width=2,stroke_fill="black")
